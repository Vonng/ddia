...
互联网和数据中心（通常是以太网）中的大多数内部网络都是 **异步分组网络（asynchronous packet networks）**。在这种网络中，一个节点可以向另一个节点发送一个消息（一个数据包），但是网络不能保证它什么时候到达，或者是否到达。如果你发送请求并期待响应，则很多事情可能会出错（其中一些如 {{< xref fig="8-1" page="/ch8" anchor="fig_ddia_0801" >}}图 8-1{{< /xref >}} 所示）：
...
{{< fig num="8-1" id="fig_ddia_0801" src="/v1/ddia_0801.png" caption="如果发送请求并没有得到响应，则无法区分（a）请求是否丢失，（b）远程节点是否关闭，或（c）响应是否丢失。" class="ddia-figure ddia-figure--panorama" width="2880" height="898" />}}
...
* 如果多个不同的节点同时尝试将数据包发送到同一目的地，则网络交换机必须将它们排队并将它们逐个送入目标网络链路（如 {{< xref fig="8-2" page="/ch8" anchor="fig_ddia_0802" >}}图 8-2{{< /xref >}} 所示）。在繁忙的网络链路上，数据包可能需要等待一段时间才能获得一个插槽（这称为网络拥塞）。如果传入的数据太多，交换机队列填满，数据包将被丢弃，因此需要重新发送数据包 - 即使网络运行良好。
...
{{< fig num="8-2" id="fig_ddia_0802" src="/v1/ddia_0802.png" caption="如果有多台机器将网络流量发送到同一目的地，则其交换机队列可能会被填满。在这里，端口 1,2 和 4 都试图发送数据包到端口 3" class="ddia-figure ddia-figure--wide" width="2880" height="1081" />}}
...
{{< xref fig="8-3" page="/ch8" anchor="fig_ddia_0803" >}}图 8-3{{< /xref >}} 显示了在具有多主复制的数据库中对时钟的危险使用（该例子类似于 {{< xref fig="5-9" page="/ch5" anchor="fig_ddia_0509" >}}图 5-9{{< /xref >}}）。客户端 A 在节点 1 上写入 `x = 1`；写入被复制到节点 3；客户端 B 在节点 3 上增加 x（我们现在有 `x = 2`）；最后这两个写入都被复制到节点 2。

{{< fig num="8-3" id="fig_ddia_0803" src="/v1/ddia_0803.png" caption="客户端 B 的写入比客户端 A 的写入要晚，但是 B 的写入具有较早的时间戳。" class="ddia-figure ddia-figure--wide" width="2880" height="1325" />}}

在 {{< xref fig="8-3" page="/ch8" anchor="fig_ddia_0803" >}}图 8-3{{< /xref >}} 中，当一个写入被复制到其他节点时，它会根据发生写入的节点上的日历时钟标记一个时间戳。在这个例子中，时钟同步是非常好的：节点 1 和节点 3 之间的偏差小于 3ms，这可能比你在实践中能预期的更好。

尽管如此，{{< xref fig="8-3" page="/ch8" anchor="fig_ddia_0803" >}}图 8-3{{< /xref >}} 中的时间戳却无法正确排列事件：写入 `x = 1` 的时间戳为 42.004 秒，但写入 `x = 2` 的时间戳为 42.003 秒，即使 `x = 2` 在稍后出现。当节点 2 接收到这两个事件时，会错误地推断出 `x = 1` 是最近的值，而丢弃写入 `x = 2`。效果上表现为，客户端 B 的增量操作会丢失。
...
* LWW 无法区分 **高频顺序写入**（在 {{< xref fig="8-3" page="/ch8" anchor="fig_ddia_0803" >}}图 8-3{{< /xref >}} 中，客户端 B 的增量操作 **一定** 发生在客户端 A 的写入之后）和 **真正并发写入**（写入者意识不到其他写入者）。需要额外的因果关系跟踪机制（例如版本向量），以防止违背因果关系（请参阅 “[检测并发写入](/v1/ch5#检测并发写入)”）。
...
例如，{{< xref fig="8-4" page="/ch8" anchor="fig_ddia_0804" >}}图 8-4{{< /xref >}} 显示了由于不正确的锁实现导致的数据损坏错误。（这个错误不仅仅是理论上的：HBase 曾经有这个问题【74,75】）假设你要确保一个存储服务中的文件一次只能被一个客户访问，因为如果多个客户试图对此写入，该文件将被损坏。你尝试通过在访问文件之前要求客户端从锁定服务获取租约来实现此目的。

{{< fig num="8-4" id="fig_ddia_0804" src="/v1/ddia_0804.png" caption="分布式锁的实现不正确：客户端 1 认为它仍然具有有效的租约，即使它已经过期，从而破坏了存储中的文件" class="ddia-figure ddia-figure--wide" width="2880" height="1057" />}}
...
当使用锁或租约来保护对某些资源（如 {{< xref fig="8-4" page="/ch8" anchor="fig_ddia_0804" >}}图 8-4{{< /xref >}} 中的文件存储）的访问时，需要确保一个被误认为自己是 “天选者” 的节点不能扰乱系统的其它部分。实现这一目标的一个相当简单的技术就是 **防护（fencing）**，如 {{< xref fig="8-5" page="/ch8" anchor="fig_ddia_0805" >}}图 8-5{{< /xref >}} 所示

{{< fig num="8-5" id="fig_ddia_0805" src="/v1/ddia_0805.png" caption="只允许以增加防护令牌的顺序进行写操作，从而保证存储安全" class="ddia-figure ddia-figure--wide" width="2880" height="1057" />}}

我们假设每次锁定服务器授予锁或租约时，它还会返回一个 **防护令牌（fencing token）**，这个数字在每次授予锁定时都会增加（例如，由锁定服务增加）。然后，我们可以要求客户端每次向存储服务发送写入请求时，都必须包含当前的防护令牌。

在 {{< xref fig="8-5" page="/ch8" anchor="fig_ddia_0805" >}}图 8-5{{< /xref >}} 中，客户端 1 以 33 的令牌获得租约，但随后进入一个长时间的停顿并且租约到期。客户端 2 以 34 的令牌（该数字总是增加）获取租约，然后将其写入请求发送到存储服务，包括 34 的令牌。稍后，客户端 1 恢复生机并将其写入存储服务，包括其令牌值 33。但是，存储服务器会记住它已经处理了一个具有更高令牌编号（34）的写入，因此它会拒绝带有令牌 33 的请求。
