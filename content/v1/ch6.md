...
一个节点可能存储多个分区。如果使用主从复制模型，则分区和复制的组合如 {{< xref fig="6-1" page="/ch6" anchor="fig_ddia_0601" >}}图 6-1{{< /xref >}} 所示。每个分区领导者（主库）被分配给一个节点，追随者（从库）被分配给其他节点。每个节点可能是某些分区的主库，同时是其他分区的从库。

我们在 [第五章](/v1/ch5) 讨论的关于数据库复制的所有内容同样适用于分区的复制。大多数情况下，分区方案的选择与复制方案的选择是独立的，为简单起见，本章中将忽略复制。

{{< fig num="6-1" id="fig_ddia_0601" src="/v1/ddia_0601.png" caption="组合使用复制和分区：每个节点充当某些分区的主库，其他分区充当从库。" class="ddia-figure ddia-figure--wide" width="2880" height="1496" />}}
...
一种分区的方法是为每个分区指定一块连续的键范围（从最小值到最大值），如纸质百科全书的卷（{{< xref fig="6-2" page="/ch6" anchor="fig_ddia_0602" >}}图 6-2{{< /xref >}}）。如果知道范围之间的边界，则可以轻松确定哪个分区包含某个值。如果你还知道分区所在的节点，那么可以直接向相应的节点发出请求（对于百科全书而言，就像从书架上选取正确的书籍）。

{{< fig num="6-2" id="fig_ddia_0602" src="/v1/ddia_0602.png" caption="印刷版百科全书按照关键字范围进行分区" class="ddia-figure ddia-figure--wide" width="2880" height="1023" />}}

键的范围不一定均匀分布，因为数据也很可能不均匀分布。例如在 {{< xref fig="6-2" page="/ch6" anchor="fig_ddia_0602" >}}图 6-2{{< /xref >}} 中，第 1 卷包含以 A 和 B 开头的单词，但第 12 卷则包含以 T、U、V、X、Y 和 Z 开头的单词。只是简单的规定每个卷包含两个字母会导致一些卷比其他卷大。为了均匀分配数据，分区边界需要依据数据调整。
...
一旦你有一个合适的键散列函数，你可以为每个分区分配一个散列范围（而不是键的范围），每个通过哈希散列落在分区范围内的键将被存储在该分区中。如 {{< xref fig="6-3" page="/ch6" anchor="fig_ddia_0603" >}}图 6-3{{< /xref >}} 所示。

{{< fig num="6-3" id="fig_ddia_0603" src="/v1/ddia_0603.png" caption="按哈希键分区" class="ddia-figure ddia-figure--panorama" width="2880" height="901" />}}
...
假设你正在经营一个销售二手车的网站（如 {{< xref fig="6-4" page="/ch6" anchor="fig_ddia_0604" >}}图 6-4{{< /xref >}} 所示）。每个列表都有一个唯一的 ID—— 称之为文档 ID—— 并且用文档 ID 对数据库进行分区（例如，分区 0 中的 ID 0 到 499，分区 1 中的 ID 500 到 999 等）。
...
{{< fig num="6-4" id="fig_ddia_0604" src="/v1/ddia_0604.png" caption="基于文档的次级索引进行分区" class="ddia-figure ddia-figure--wide" width="2880" height="1365" />}}
...
但是，从文档分区索引中读取需要注意：除非你对文档 ID 做了特别的处理，否则没有理由将所有具有特定颜色或特定品牌的汽车放在同一个分区中。在 {{< xref fig="6-4" page="/ch6" anchor="fig_ddia_0604" >}}图 6-4{{< /xref >}} 中，红色汽车出现在分区 0 和分区 1 中。因此，如果要搜索红色汽车，则需要将查询发送到所有分区，并合并所有返回的结果。
...
{{< xref fig="6-5" page="/ch6" anchor="fig_ddia_0605" >}}图 6-5{{< /xref >}} 描述了这可能是什么样子：来自所有分区的红色汽车在红色索引中，并且索引是分区的，首字母从 `a` 到 `r` 的颜色在分区 0 中，`s` 到 `z` 的在分区 1。汽车制造商的索引也与之类似（分区边界在 `f` 和 `h` 之间）。

{{< fig num="6-5" id="fig_ddia_0605" src="/v1/ddia_0605.png" caption="基于关键词对次级索引进行分区" class="ddia-figure ddia-figure--wide" width="2880" height="1223" />}}
...
我们在前面说过（{{< xref fig="6-3" page="/ch6" anchor="fig_ddia_0603" >}}图 6-3{{< /xref >}}），最好将可能的散列分成不同的范围，并将每个范围分配给一个分区（例如，如果 $0 ≤ hash(key)< b_0$，则将键分配给分区 0，如果 $b_0 ≤ hash(key) < b_1$，则分配给分区 1）
...
现在，如果一个节点被添加到集群中，新节点可以从当前每个节点中 **窃取** 一些分区，直到分区再次公平分配。这个过程如 {{< xref fig="6-6" page="/ch6" anchor="fig_ddia_0606" >}}图 6-6{{< /xref >}} 所示。如果从集群中删除一个节点，则会发生相反的情况。

只有分区在节点之间的移动。分区的数量不会改变，键所指定的分区也不会改变。唯一改变的是分区所在的节点。这种变更并不是即时的 — 在网络上传输大量的数据需要一些时间 — 所以在传输过程中，原有分区仍然会接受读写操作。

{{< fig num="6-6" id="fig_ddia_0606" src="/v1/ddia_0606.png" caption="将新节点添加到每个节点具有多个分区的数据库集群。" class="ddia-figure ddia-figure--wide" width="2880" height="1557" />}}
...
{{< fig num="6-7" id="fig_ddia_0607" src="/v1/ddia_0607.png" caption="将请求路由到正确节点的三种不同方式。" class="ddia-figure ddia-figure--wide" width="2880" height="1281" />}}

这是一个具有挑战性的问题，因为重要的是所有参与者都达成共识 - 否则请求将被发送到错误的节点，得不到正确的处理。在分布式系统中有达成共识的协议，但很难正确地实现（见 [第九章](/v1/ch9)）。

许多分布式数据系统都依赖于一个独立的协调服务，比如 ZooKeeper 来跟踪集群元数据，如 {{< xref fig="6-8" page="/ch6" anchor="fig_ddia_0608" >}}图 6-8{{< /xref >}} 所示。每个节点在 ZooKeeper 中注册自己，ZooKeeper 维护分区到节点的可靠映射。其他参与者（如路由层或分区感知客户端）可以在 ZooKeeper 中订阅此信息。只要分区分配发生了改变，或者集群中添加或删除了一个节点，ZooKeeper 就会通知路由层使路由信息保持最新状态。

{{< fig num="6-8" id="fig_ddia_0608" src="/v1/ddia_0608.png" caption="使用 ZooKeeper 跟踪分区分配给节点。" class="ddia-figure ddia-figure--wide" width="2880" height="1260" />}}

例如，LinkedIn的Espresso使用Helix 【31】进行集群管理（依靠ZooKeeper），实现了如{{< xref fig="6-8" page="/ch6" anchor="fig_ddia_0608" >}}图6-8{{< /xref >}}所示的路由层。HBase、SolrCloud和Kafka也使用ZooKeeper来跟踪分区分配。MongoDB具有类似的体系结构，但它依赖于自己的**配置服务器（config server）** 实现和mongos守护进程作为路由层。

Cassandra 和 Riak 采取不同的方法：他们在节点之间使用 **流言协议（gossip protocol）** 来传播集群状态的变化。请求可以发送到任意节点，该节点会转发到包含所请求的分区的适当节点（{{< xref fig="6-7" page="/ch6" anchor="fig_ddia_0607" >}}图 6-7{{< /xref >}} 中的方法 1）。这个模型在数据库节点中增加了更多的复杂性，但是避免了对像 ZooKeeper 这样的外部协调服务的依赖。
