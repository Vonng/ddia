...
{{< xref fig="7-1" page="/ch7" anchor="fig_ddia_0701" >}}图 7-1{{< /xref >}} 是这类问题的一个简单例子。假设你有两个客户端同时在数据库中增长一个计数器。（假设数据库没有内建的自增操作）每个客户端需要读取计数器的当前值，加 1 ，再回写新值。{{< xref fig="7-1" page="/ch7" anchor="fig_ddia_0701" >}}图 7-1{{< /xref >}} 中，因为发生了两次增长，计数器应该从 42 增至 44；但由于竞态条件，实际上只增至 43 。
...
{{< fig num="7-1" id="fig_ddia_0701" src="/v1/ddia_0701.png" caption="两个客户之间的竞争状态同时递增计数器" class="ddia-figure ddia-figure--panorama" width="2880" height="832" />}}
...
这些定义假设你想同时修改多个对象（行，文档，记录）。通常需要 **多对象事务（multi-object transaction）** 来保持多块数据同步。{{< xref fig="7-2" page="/ch7" anchor="fig_ddia_0702" >}}图 7-2{{< /xref >}} 展示了一个来自电邮应用的例子。执行以下查询来显示用户未读邮件数量：
...
在 {{< xref fig="7-2" page="/ch7" anchor="fig_ddia_0702" >}}图 7-2{{< /xref >}} 中，用户 2 遇到异常情况：邮件列表里显示有未读消息，但计数器显示为零未读消息，因为计数器增长还没有发生 [^ii]。隔离性可以避免这个问题：通过确保用户 2 要么同时看到新邮件和增长后的计数器，要么都看不到，而不是一个前后矛盾的中间结果。

[^ii]: 可以说邮件应用中的错误计数器并不是什么特别重要的问题。但换种方式来看，你可以把未读计数器换成客户账户余额，把邮件收发看成支付交易。

{{< fig num="7-2" id="fig_ddia_0702" src="/v1/ddia_0702.png" caption="违反隔离性：一个事务读取另一个事务的未被执行的写入（“脏读”）。" class="ddia-figure ddia-figure--wide" width="2880" height="1149" />}}

{{< xref fig="7-3" page="/ch7" anchor="fig_ddia_0703" >}}图 7-3{{< /xref >}} 说明了对原子性的需求：如果在事务过程中发生错误，邮箱和未读计数器的内容可能会失去同步。在原子事务中，如果对计数器的更新失败，事务将被中止，并且插入的电子邮件将被回滚。

{{< fig num="7-3" id="fig_ddia_0703" src="/v1/ddia_0703.png" caption="原子性确保发生错误时，事务先前的任何写入都会被撤消，以避免状态不一致" class="ddia-figure ddia-figure--panorama" width="2880" height="763" />}}
...
一些数据库也提供更复杂的原子操作 [^iv]，例如自增操作，这样就不再需要像 {{< xref fig="7-1" page="/ch7" anchor="fig_ddia_0701" >}}图 7-1{{< /xref >}} 那样的读取 - 修改 - 写入序列了。同样流行的是 **[比较和设置（CAS, compare-and-set）](#比较并设置（CAS）)** 操作，仅当值没有被其他并发修改过时，才允许执行写操作。
...
* 在文档数据模型中，需要一起更新的字段通常在同一个文档中，这被视为单个对象 —— 更新单个文档时不需要多对象事务。但是，缺乏连接功能的文档数据库会鼓励非规范化（请参阅 “[关系型数据库与文档数据库在今日的对比](/v1/ch2#关系型数据库与文档数据库在今日的对比)”）。当需要更新非规范化的信息时，如 {{< xref fig="7-2" page="/ch7" anchor="fig_ddia_0702" >}}图 7-2{{< /xref >}} 所示，需要一次更新多个文档。事务在这种情况下非常有用，可以防止非规范化的数据不同步。
...
在 **读已提交** 隔离级别运行的事务必须防止脏读。这意味着事务的任何写入操作只有在该事务提交时才能被其他人看到（然后所有的写入操作都会立即变得可见）。如 {{< xref fig="7-4" page="/ch7" anchor="fig_ddia_0704" >}}图 7-4{{< /xref >}} 所示，用户 1 设置了 `x = 3`，但用户 2 的 `get x` 仍旧返回旧值 2 （当用户 1 尚未提交时）。

{{< fig num="7-4" id="fig_ddia_0704" src="/v1/ddia_0704.png" caption="没有脏读：用户 2 只有在用户 1 的事务已经提交后才能看到 x 的新值。" class="ddia-figure ddia-figure--panorama" width="2880" height="832" />}}

为什么要防止脏读，有几个原因：

- 如果事务需要更新多个对象，脏读取意味着另一个事务可能会只看到一部分更新。例如，在 {{< xref fig="7-2" page="/ch7" anchor="fig_ddia_0702" >}}图 7-2{{< /xref >}} 中，用户看到新的未读电子邮件，但看不到更新的计数器。这就是电子邮件的脏读。看到处于部分更新状态的数据库会让用户感到困惑，并可能导致其他事务做出错误的决定。
- 如果事务中止，则所有写入操作都需要回滚（如 {{< xref fig="7-3" page="/ch7" anchor="fig_ddia_0703" >}}图 7-3{{< /xref >}} 所示）。如果数据库允许脏读，那就意味着一个事务可能会看到稍后需要回滚的数据，即从未实际提交给数据库的数据。想想后果就让人头大。
...
- 如果事务更新多个对象，脏写会导致不好的结果。例如，考虑 {{< xref fig="7-5" page="/ch7" anchor="fig_ddia_0705" >}}图 7-5{{< /xref >}}，以一个二手车销售网站为例，Alice 和 Bob 两个人同时试图购买同一辆车。购买汽车需要两次数据库写入：网站上的商品列表需要更新，以反映买家的购买，销售发票需要发送给买家。在 {{< xref fig="7-5" page="/ch7" anchor="fig_ddia_0705" >}}图 7-5{{< /xref >}} 的情况下，销售是属于 Bob 的（因为他成功更新了商品列表），但发票却寄送给了 Alice（因为她成功更新了发票表）。读已提交会防止这样的事故。
- 但是，读已提交并不能防止 {{< xref fig="7-1" page="/ch7" anchor="fig_ddia_0701" >}}图 7-1{{< /xref >}} 中两个计数器增量之间的竞争状态。在这种情况下，第二次写入发生在第一个事务提交后，所以它不是一个脏写。这仍然是不正确的，但是出于不同的原因，在 “[防止丢失更新](#防止丢失更新)” 中将讨论如何使这种计数器增量安全。

{{< fig num="7-5" id="fig_ddia_0705" src="/v1/ddia_0705.png" caption="如果存在脏写，来自不同事务的冲突写入可能会混淆在一起" class="ddia-figure ddia-figure--wide" width="2880" height="1301" />}}
...
出于这个原因，大多数数据库 [^vi] 使用 {{< xref fig="7-4" page="/ch7" anchor="fig_ddia_0704" >}}图 7-4{{< /xref >}} 的方式防止脏读：对于写入的每个对象，数据库都会记住旧的已提交值，和由当前持有写入锁的事务设置的新值。当事务正在进行时，任何其他读取对象的事务都会拿到旧值。只有当新值提交后，事务才会切换到读取新值。
...
但是在使用此隔离级别时，仍然有很多地方可能会产生并发错误。例如 {{< xref fig="7-6" page="/ch7" anchor="fig_ddia_0706" >}}图 7-6{{< /xref >}} 说明了读已提交时可能发生的问题。

{{< fig num="7-6" id="fig_ddia_0706" src="/v1/ddia_0706.png" caption="读取偏差：Alice 观察数据库处于不一致的状态" class="ddia-figure ddia-figure--wide" width="2880" height="1301" />}}
...
为了实现快照隔离，数据库使用了我们看到的用于防止 {{< xref fig="7-4" page="/ch7" anchor="fig_ddia_0704" >}}图 7-4{{< /xref >}} 中的脏读的机制的一般化。数据库必须可能保留一个对象的几个不同的提交版本，因为各种正在进行的事务可能需要看到数据库在不同的时间点的状态。因为它同时维护着单个对象的多个版本，所以这种技术被称为 **多版本并发控制（MVCC, multi-version concurrency control）**。
...
{{< xref fig="7-7" page="/ch7" anchor="fig_ddia_0707" >}}图 7-7{{< /xref >}} 说明了 PostgreSQL 如何实现基于 MVCC 的快照隔离【31】（其他实现类似）。当一个事务开始时，它被赋予一个唯一的，永远增长 [^vii] 的事务 ID（`txid`）。每当事务向数据库写入任何内容时，它所写入的数据都会被标记上写入者的事务 ID。

[^vii]: 事实上，事务 ID 是 32 位整数，所以大约会在 40 亿次事务之后溢出。PostgreSQL 的 Vacuum 过程会清理老旧的事务 ID，确保事务 ID 溢出（回卷）不会影响到数据。

{{< fig num="7-7" id="fig_ddia_0707" src="/v1/ddia_0707.png" caption="使用多版本对象实现快照隔离" class="ddia-figure ddia-figure--standard" width="2880" height="2181" />}}
...
`UPDATE` 操作在内部翻译为 `DELETE` 和 `INSERT` 。例如，在 {{< xref fig="7-7" page="/ch7" anchor="fig_ddia_0707" >}}图 7-7{{< /xref >}} 中，事务 13 从账户 2 中扣除 100 美元，将余额从 500 美元改为 400 美元。实际上包含两条账户 2 的记录：余额为 \$500 的行被标记为 **被事务 13 删除**，余额为 \$400 的行 **由事务 13 创建**。
...
这些规则适用于创建和删除对象。在 {{< xref fig="7-7" page="/ch7" anchor="fig_ddia_0707" >}}图 7-7{{< /xref >}} 中，当事务 12 从账户 2 读取时，它会看到 \$500 的余额，因为 \$500 余额的删除是由事务 13 完成的（根据规则 3，事务 12 看不到事务 13 执行的删除），且 400 美元记录的创建也是不可见的（按照相同的规则）。
...
并发的写入事务之间还有其他几种有趣的冲突。其中最著名的是 **丢失更新（lost update）** 问题，如 {{< xref fig="7-1" page="/ch7" anchor="fig_ddia_0701" >}}图 7-1{{< /xref >}} 所示，以两个并发计数器增量为例。
...
相反，如 “[检测并发写入](/v1/ch5#检测并发写入)” 一节所述，这种复制数据库中的一种常见方法是允许并发写入创建多个冲突版本的值（也称为兄弟值），并使用应用代码或特殊数据结构在事实发生之后解决和合并这些版本。
...
现在想象一下，Alice 和 Bob 是两位值班医生。两人都感到不适，所以他们都决定请假。不幸的是，他们恰好在同一时间点击按钮下班。{{< xref fig="7-8" page="/ch7" anchor="fig_ddia_0708" >}}图 7-8{{< /xref >}} 说明了接下来的事情。

{{< fig num="7-8" id="fig_ddia_0708" src="/v1/ddia_0708.png" caption="写入偏差导致应用程序错误的示例" class="ddia-figure ddia-figure--standard" width="2880" height="2019" />}}
...
出于这个原因，具有单线程串行事务处理的系统不允许交互式的多语句事务。取而代之，应用程序必须提前将整个事务代码作为存储过程提交给数据库。这些方法之间的差异如 {{< xref fig="7-9" page="/ch7" anchor="fig_ddia_0709" >}}图 7-9{{< /xref >}} 所示。如果事务所需的所有数据都在内存中，则存储过程可以非常快地执行，而不用等待任何网络或磁盘 I/O。

{{< fig num="7-9" id="fig_ddia_0709" src="/v1/ddia_0709.png" caption="交互式事务和存储过程之间的区别（使用图 7-8 的示例事务）" class="ddia-figure ddia-figure--wide" width="2880" height="1663" />}}
...
- 如果事务 A 写入了一个对象，并且事务 B 想要读取该对象，则 B 必须等到 A 提交或中止才能继续（像 {{< xref fig="7-1" page="/ch7" anchor="fig_ddia_0701" >}}图 7-1{{< /xref >}} 那样读取旧版本的对象在 2PL 下是不可接受的）。
...
回想一下，快照隔离通常是通过多版本并发控制（MVCC；见 {{< xref fig="7-10" page="/ch7" anchor="fig_ddia_0710" >}}图 7-10{{< /xref >}}）来实现的。当一个事务从 MVCC 数据库中的一致快照读时，它将忽略取快照时尚未提交的任何其他事务所做的写入。在 {{< xref fig="7-10" page="/ch7" anchor="fig_ddia_0710" >}}图 7-10{{< /xref >}} 中，事务 43 认为 Alice 的 `on_call = true` ，因为事务 42（修改 Alice 的待命状态）未被提交。然而，在事务 43 想要提交时，事务 42 已经提交。这意味着在读一致性快照时被忽略的写入已经生效，事务 43 的前提不再为真。

{{< fig num="7-10" id="fig_ddia_0710" src="/v1/ddia_0710.png" caption="检测事务何时从 MVCC 快照读取过时的值" class="ddia-figure ddia-figure--standard" width="2880" height="1748" />}}
...
第二种情况要考虑的是另一个事务在读取数据之后修改数据。这种情况如 {{< xref fig="7-11" page="/ch7" anchor="fig_ddia_0711" >}}图 7-11{{< /xref >}} 所示。

{{< fig num="7-11" id="fig_ddia_0711" src="/v1/ddia_0711.png" caption="在可串行化快照隔离中，检测一个事务何时修改另一个事务的读取。" class="ddia-figure ddia-figure--standard" width="2880" height="1792" />}}

在两阶段锁定的上下文中，我们讨论了索引范围锁（请参阅 “[索引范围锁](#索引范围锁)”），它允许数据库锁定与某个搜索查询匹配的所有行的访问权，例如 `WHERE shift_id = 1234`。可以在这里使用类似的技术，除了 SSI 锁不会阻塞其他事务。

在 {{< xref fig="7-11" page="/ch7" anchor="fig_ddia_0711" >}}图 7-11{{< /xref >}} 中，事务 42 和 43 都在班次 1234 查找值班医生。如果在 `shift_id` 上有索引，则数据库可以使用索引项 1234 来记录事务 42 和 43 读取这个数据的事实。（如果没有索引，这个信息可以在表级别进行跟踪）。这个信息只需要保留一段时间：在一个事务完成（提交或中止），并且所有的并发事务完成之后，数据库就可以忘记它读取的数据了。

当事务写入数据库时，它必须在索引中查找最近曾读取受影响数据的其他事务。这个过程类似于在受影响的键范围上获取写锁，但锁并不会阻塞事务直到其他读事务完成，而是像警戒线一样只是简单通知其他事务：你们读过的数据可能不是最新的啦。

在 {{< xref fig="7-11" page="/ch7" anchor="fig_ddia_0711" >}}图 7-11{{< /xref >}} 中，事务 42 通知事务 43 其先前读已过时，反之亦然。事务 42 首先提交并成功，尽管事务 43 的写影响了 42 ，但因为事务 43 尚未提交，所以写入尚未生效。然而当事务 43 想要提交时，来自事务 42 的冲突写入已经被提交，所以事务 43 必须中止。
