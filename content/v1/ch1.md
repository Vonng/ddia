...
例如，如果将缓存（应用管理的缓存层，Memcached 或同类产品）和全文搜索（全文搜索服务器，例如 Elasticsearch 或 Solr）功能从主数据库剥离出来，那么使缓存 / 索引与主数据库保持同步通常是应用代码的责任。{{< xref fig="1-1" page="/ch1" anchor="fig_ddia_0101" >}}图 1-1{{< /xref >}} 给出了这种架构可能的样子（细节将在后面的章节中详细介绍）。

{{< fig num="1-1" id="fig_ddia_0101" src="/v1/ddia_0101.png" caption="一个可能的组合使用多个组件的数据系统架构" class="ddia-figure ddia-figure--standard" width="2880" height="2049" />}}
...
1. 发布推文时，只需将新推文插入全局推文集合即可。当一个用户请求自己的主页时间线时，首先查找他关注的所有人，查询这些被关注用户发布的推文并按时间顺序合并。在如 {{< xref fig="1-2" page="/ch1" anchor="fig_ddia_0102" >}}图 1-2{{< /xref >}} 所示的关系型数据库中，可以编写这样的查询：
...
{{< fig num="1-2" id="fig_ddia_0102" src="/v1/ddia_0102.png" caption="推特主页时间线的关系型模式简单实现" class="ddia-figure ddia-figure--wide" width="2880" height="1037" />}}

2. 为每个用户的主页时间线维护一个缓存，就像每个用户的推文收件箱（{{< xref fig="1-3" page="/ch1" anchor="fig_ddia_0103" >}}图 1-3{{< /xref >}}）。当一个用户发布推文时，查找所有关注该用户的人，并将新的推文插入到每个主页时间线缓存中。因此读取主页时间线的请求开销很小，因为结果已经提前计算好了。

{{< fig num="1-3" id="fig_ddia_0103" src="/v1/ddia_0103.png" caption="用于分发推特至关注者的数据流水线，2012 年 11 月的负载参数【16】" class="ddia-figure ddia-figure--wide" width="2880" height="1037" />}}
...
在 {{< xref fig="1-4" page="/ch1" anchor="fig_ddia_0104" >}}图 1-4{{< /xref >}} 中，每个灰条代表一次对服务的请求，其高度表示请求花费了多长时间。大多数请求是相当快的，但偶尔会出现需要更长的时间的异常值。这也许是因为缓慢的请求实质上开销更大，例如它们可能会处理更多的数据。但即使（你认为）所有请求都花费相同时间的情况下，随机的附加延迟也会导致结果变化，例如：上下文切换到后台进程，网络数据包丢失与 TCP 重传，垃圾收集暂停，强制从磁盘读取的页面错误，服务器机架中的震动【18】，还有很多其他原因。

{{< fig num="1-4" id="fig_ddia_0104" src="/v1/ddia_0104.png" caption="展示了一个服务 100 次请求响应时间的均值与百分位数" class="ddia-figure ddia-figure--panorama" width="2880" height="805" />}}
...
为了弄清异常值有多糟糕，可以看看更高的百分位点，例如第 95、99 和 99.9 百分位点（缩写为 p95，p99 和 p999）。它们意味着 95%、99% 或 99.9% 的请求响应时间要比该阈值快，例如：如果第 95 百分位点响应时间是 1.5 秒，则意味着 100 个请求中的 95 个响应时间快于 1.5 秒，而 100 个请求中的 5 个响应时间超过 1.5 秒。如 {{< xref fig="1-4" page="/ch1" anchor="fig_ddia_0104" >}}图 1-4{{< /xref >}} 所示。
...
> #### 实践中的百分位点
>
> 在多重调用的后端服务里，高百分位数变得特别重要。即使并行调用，最终用户请求仍然需要等待最慢的并行调用完成。如 {{< xref fig="1-5" page="/ch1" anchor="fig_ddia_0105" >}}图 1-5{{< /xref >}} 所示，只需要一个缓慢的调用就可以使整个最终用户请求变慢。即使只有一小部分后端调用速度较慢，如果最终用户请求需要多个后端调用，则获得较慢调用的机会也会增加，因此较高比例的最终用户请求速度会变慢（该效果称为尾部延迟放大，即 tail latency amplification【24】）。
>
...
{{< fig num="1-5" id="fig_ddia_0105" src="/v1/ddia_0105.png" caption="当一个请求需要多个后端请求时，单个后端慢请求就会拖慢整个终端用户的请求" class="ddia-figure ddia-figure--wide" width="2880" height="1304" />}}
...
在本书后面的 [第三部分](/v1/part-iii) 中，我们将看到一种模式：几个组件协同工作以构成一个完整的系统（如 {{< xref fig="1-1" page="/ch1" anchor="fig_ddia_0101" >}}图 1-1{{< /xref >}} 中的例子）
