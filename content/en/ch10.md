...
{{< fig num="10-1" id="fig_consistency_linearizability_0" src="/fig/ddia_1001.png" caption="If this database were linearizable, then either Alice's read would return 1 instead of 0, or Bob's read would return 0 instead of 1." />}}
...
{{< fig num="10-2" id="fig_consistency_linearizability_1" src="/fig/ddia_1002.png" caption="Alice observes that x = 0 and y = 1, while Bob observes that x = 1 and y = 0. It's as if Alice's and Bob's computers disagree on the order in which the writes happened." />}}
...
{{< fig num="10-3" id="fig_consistency_linearizability_2" src="/fig/ddia_1003.png" caption="If Alice and Bob had perfect clocks, linearizability would require that x = 1 is returned, since the read of x begins after the write x = 1 completes." />}}
...
{{< fig num="10-4" id="fig_consistency_linearizability_3" src="/fig/ddia_1004.png" caption="The read of x is concurrent with the write x = 1. Since we don't know the exact timing of the operations, the read is allowed to return either 0 or 1." />}}
...
{{< fig num="10-5" id="fig_consistency_transcoder" src="/fig/ddia_1005.png" caption="A system that is not linearizable: Alice and Bob see the uploaded image at different times, and thus Bob's request is based on stale data." />}}
...
{{< fig num="10-6" id="fig_consistency_leaderless" src="/fig/ddia_1006.png" caption="Quorums are not sufficient to ensure linearizability if network delays are variable." />}}
...
{{< fig num="10-7" id="fig_consistency_cap_availability" src="/fig/ddia_1007.png" caption="If clients cannot contact enough replicas due to a network partition, they cannot process writes." />}}
...
{{< fig num="10-8" id="fig_consistency_id_generator" src="/fig/ddia_1008.png" caption="Two different nodes may generate conflicting IDs." />}}
...
{{< fig num="10-9" id="fig_consistency_lamport_ts" src="/fig/ddia_1009.png" caption="Lamport timestamps provide a total ordering consistent with causality." />}}
...
{{< fig num="10-10" id="fig_consistency_permissions" src="/fig/ddia_1010.png" caption="An example of a permission system using Lamport timestamps." />}}
