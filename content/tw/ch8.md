...
如今，幾乎所有關係型資料庫和一部分非關聯式資料庫都支援事務。它們大多沿襲 1975 年由第一個 SQL 資料庫 IBM System R 開創的風格[^2] [^3] [^4]。儘管一些實現細節發生了變化，但 50 年來總體思路幾乎原封未動：MySQL、PostgreSQL、Oracle 和 SQL Server 等資料庫的事務支援與 System R 異乎尋常地相似。
...
{{< xref fig="8-1" page="/ch8" anchor="fig_transactions_increment" >}}圖 8-1{{< /xref >}}給出了這類問題的一個簡單例子。假設兩個客戶端同時遞增資料庫中的一個計數器。每個客戶端都要讀取當前值，將其加 1，再把新值寫回去（假設資料庫沒有內建的遞增操作）。在{{< xref fig="8-1" page="/ch8" anchor="fig_transactions_increment" >}}圖 8-1{{< /xref >}}中，計數器經過兩次遞增，本應從 42 變成 44，卻因競態條件最終只變成了 43。

{{< fig num="8-1" id="fig_transactions_increment" src="/fig/ddia_0801.png" caption="兩個客戶端併發遞增計數器之間的競態條件。" class="ddia-figure ddia-figure--panorama" width="2880" height="832" />}}
...
這些定義假設你想同時修改多個物件（行、文件或記錄）。當多項資料需要保持同步時，通常就要用到這種*多物件事務*。{{< xref fig="8-2" page="/ch8" anchor="fig_transactions_read_uncommitted" >}}圖 8-2{{< /xref >}}展示了一個電子郵件應用的例子。要顯示使用者的未讀郵件數，可以執行如下查詢：

```
SELECT COUNT(*) FROM emails WHERE recipient_id = 2 AND unread_flag = true
```

{{< fig num="8-2" id="fig_transactions_read_uncommitted" src="/fig/ddia_0802.png" caption="違反隔離性：一個事務讀取另一個事務的未提交寫入（“髒讀”）。" class="ddia-figure ddia-figure--wide" width="2880" height="1149" />}}
...
在{{< xref fig="8-2" page="/ch8" anchor="fig_transactions_read_uncommitted" >}}圖 8-2{{< /xref >}}中，使用者 2 遇到了異常：郵箱列表中已經出現一封未讀郵件，未讀計數卻仍為零，因為計數器尚未遞增。（如果電子郵件應用中的錯誤計數無關緊要，不妨把未讀計數換成客戶賬戶餘額，把郵件換成支付事務。）隔離性可以防止這個問題：它保證使用者 2 要麼同時看到新插入的郵件和更新後的計數，要麼兩者都看不到，而不會看到不一致的中間狀態。

{{< xref fig="8-3" page="/ch8" anchor="fig_transactions_atomicity" >}}圖 8-3{{< /xref >}}說明了為何需要原子性：如果事務進行到一半時發生錯誤，郵箱內容和未讀計數可能失去同步。在原子事務中，如果計數器更新失敗，事務就會中止，已經插入的郵件也會回滾。

{{< fig num="8-3" id="fig_transactions_atomicity" src="/fig/ddia_0803.png" caption="原子性確保如果發生錯誤，該事務的任何先前寫入都會被撤消，以避免不一致的狀態。" class="ddia-figure ddia-figure--panorama" width="2880" height="763" />}}
...
有些資料庫還提供更複雜的原子操作，例如遞增操作，從而不必執行{{< xref fig="8-1" page="/ch8" anchor="fig_transactions_increment" >}}圖 8-1{{< /xref >}}中的讀取—修改—寫入迴圈。同樣常見的還有*條件寫入*：只有當該值未被其他人併發修改時，寫入才會發生（參見[“條件寫入（比較並設定）”](/tw/ch8#sec_transactions_compare_and_set)）。它類似於共享記憶體併發中的比較並設定或比較並交換（CAS）操作。
...
* 在文件資料模型中，需要一同更新的欄位通常位於同一份文件內，而文件被視為單個物件，因此更新一份文件無須多物件事務。不過，缺乏連線功能的文件資料庫也鼓勵反正規化（參見[“何時使用哪種模型”](/tw/ch3#sec_datamodels_document_summary)）。當反正規化的資訊需要更新時——例如{{< xref fig="8-2" page="/ch8" anchor="fig_transactions_read_uncommitted" >}}圖 8-2{{< /xref >}}中的情形——就必須一次更新多份文件。事務在這裡很有用，可以防止反正規化資料彼此失去同步。
...
執行在讀已提交隔離級別下的事務必須防止髒讀。這意味著，一個事務的所有寫入都要等到該事務提交時，才會同時對其他事務可見。{{< xref fig="8-4" page="/ch8" anchor="fig_transactions_read_committed" >}}圖 8-4{{< /xref >}}中，使用者 1 已將 *x* 設為 3，但由於其事務尚未提交，使用者 2 的 *get x* 仍返回舊值 2。

{{< fig num="8-4" id="fig_transactions_read_committed" src="/fig/ddia_0804.png" caption="沒有髒讀：使用者 2 只有在使用者 1 的事務提交後才能看到 x 的新值。" class="ddia-figure ddia-figure--panorama" width="2880" height="832" />}}

防止髒讀有以下幾個好處：

* 如果事務要更新多行，髒讀會讓另一個事務只看到其中一部分更新。例如在{{< xref fig="8-2" page="/ch8" anchor="fig_transactions_read_uncommitted" >}}圖 8-2{{< /xref >}}中，使用者看到了新到的未讀郵件，卻沒有看到更新後的計數器。這就是對郵件的髒讀。資料庫呈現出部分更新的狀態，不僅令人困惑，還可能誘使其他事務作出錯誤決定。
* 如果事務中止，其所有寫入都需要回滾（如{{< xref fig="8-3" page="/ch8" anchor="fig_transactions_atomicity" >}}圖 8-3{{< /xref >}}）。允許髒讀，就意味著事務可能讀到後來被回滾、從未真正提交進資料庫的資料。凡是讀取過未提交資料的事務也必須一併中止，從而造成所謂的*級聯中止*。
...
* 如果事務更新多行，髒寫可能產生非常糟糕的結果。{{< xref fig="8-5" page="/ch8" anchor="fig_transactions_dirty_writes" >}}圖 8-5{{< /xref >}}展示了一家二手車交易網站：Aaliyah 和 Bryce 同時購買同一輛車。買車涉及兩次資料庫寫入：網站上的商品資訊要更新為買家的姓名，銷售發票也要開給買家。在{{< xref fig="8-5" page="/ch8" anchor="fig_transactions_dirty_writes" >}}圖 8-5{{< /xref >}}所示的情況下，車輛最終賣給了 Bryce（他對 `listings` 表的更新勝出），發票卻開給了 Aaliyah（她對 `invoices` 表的更新勝出）。讀已提交可以防止這種事故。
* 不過，讀已提交*不能*防止{{< xref fig="8-1" page="/ch8" anchor="fig_transactions_increment" >}}圖 8-1{{< /xref >}}中兩個計數器遞增操作之間的競態條件。這裡的第二次寫入發生在第一個事務提交之後，所以並非髒寫。結果仍然有誤，只是原因不同；[“防止丟失更新”](/tw/ch8#sec_transactions_lost_update)將介紹如何安全地執行這類計數器遞增操作。

{{< fig num="8-5" id="fig_transactions_dirty_writes" src="/fig/ddia_0805.png" caption="發生髒寫時，不同事務的衝突寫入可能混雜在一起。" class="ddia-figure ddia-figure--wide" width="2658" height="1200" />}}
...
更常見的防髒讀辦法如{{< xref fig="8-4" page="/ch8" anchor="fig_transactions_read_committed" >}}圖 8-4{{< /xref >}}所示：對於每一行寫入，資料庫同時保留舊的已提交值，以及當前持有寫鎖的事務寫入的新值。該事務進行期間，其他事務讀取這一行時只會得到舊值；等新值提交後，才改為讀取新值（詳見[“多版本併發控制（MVCC）”](/tw/ch8#sec_transactions_snapshot_impl)）。
...
然而，在這個隔離級別下，併發缺陷仍可能以許多方式出現。{{< xref fig="8-6" page="/ch8" anchor="fig_transactions_item_many_preceders" >}}圖 8-6{{< /xref >}}就展示了讀已提交可能遇到的一個問題。

{{< fig num="8-6" id="fig_transactions_item_many_preceders" src="/fig/ddia_0806.png" caption="讀偏差：Aaliyah 觀察到了資料庫的不一致狀態。" class="ddia-figure ddia-figure--wide" width="2658" height="1200" />}}
...
{{< xref fig="8-7" page="/ch8" anchor="fig_transactions_mvcc" >}}圖 8-7{{< /xref >}}展示了 PostgreSQL 如何基於 MVCC 實現快照隔離[^40] [^42] [^43]（其他實現與之類似）。事務啟動時會獲得一個唯一且單調遞增的事務 ID（`txid`）。事務寫入資料庫的任何資料，都會以寫入者的事務 ID 標記。（嚴格來說，PostgreSQL 的事務 ID 是 32 位整數，大約經過 40 億個事務就會溢位；`vacuum` 程序負責清理，確保溢位不會影響資料。）

{{< fig num="8-7" id="fig_transactions_mvcc" src="/fig/ddia_0807.png" caption="使用多版本併發控制實現快照隔離。" class="ddia-figure ddia-figure--standard" width="2658" height="2013" />}}
...
更新在內部會轉換成一次刪除和一次插入[^44]。例如，在{{< xref fig="8-7" page="/ch8" anchor="fig_transactions_mvcc" >}}圖 8-7{{< /xref >}}中，事務 13 從賬戶 2 扣除 100 美元，把餘額從 500 美元改為 400 美元。此時 `accounts` 表實際上包含賬戶 2 的兩行：餘額 500 美元的行由事務 13 標記為刪除，餘額 400 美元的行則由事務 13 插入。
...
這些規則既適用於插入行，也適用於刪除行。在{{< xref fig="8-7" page="/ch8" anchor="fig_transactions_mvcc" >}}圖 8-7{{< /xref >}}中，事務 12 讀取賬戶 2 時看到的餘額是 500 美元：刪除 500 美元餘額的是事務 13，而根據規則 2，事務 12 看不到事務 13 執行的刪除；同理，插入的 400 美元餘額也尚不可見。
...
以多人遊戲為例，幾個玩家都可以移動同一個棋子。這時原子操作可能還不夠，因為應用還要確保走法符合遊戲規則，而其中一些邏輯很難合理地寫成資料庫查詢。此時可以加鎖，防止兩個玩家同時移動同一個棋子，如{{< xref page="/ch8" anchor="fig_transactions_select_for_update" >}}示例 8-1{{< /xref >}}所示。

{{< example num="8-1" id="fig_transactions_select_for_update" caption="顯式鎖定行以防止丟失更新" />}}
...
假設某個班次只有 Aaliyah 和 Bryce 兩位值班醫生。兩人都身體不適，決定請假，偏偏又幾乎同時點選了退出值班的按鈕。接下來發生的事情如{{< xref fig="8-8" page="/ch8" anchor="fig_transactions_write_skew" >}}圖 8-8{{< /xref >}}所示。

{{< fig num="8-8" id="fig_transactions_write_skew" src="/fig/ddia_0808.png" caption="寫偏差導致應用程式錯誤的示例。" class="ddia-figure ddia-figure--standard" width="2658" height="1863" />}}
...
會議室預訂系統
: 假設你要保證同一間會議室在同一時段不能被重複預訂[^55]。有人發起預訂時，先檢查是否存在衝突（即同一房間是否有時間範圍重疊的預訂）；若沒有，再建立會議（參見{{< xref page="/ch8" anchor="fig_transactions_meeting_rooms" >}}示例 8-2{{< /xref >}}）。

    {{< example num="8-2" id="fig_transactions_meeting_rooms" caption="會議室預訂系統試圖避免重複預訂（在快照隔離下並不安全）" />}}
...
多人遊戲
: 在{{< xref page="/ch8" anchor="fig_transactions_select_for_update" >}}示例 8-1{{< /xref >}}中，我們用鎖防止丟失更新，也就是確保兩名玩家不能同時移動同一個棋子。但這把鎖無法阻止玩家把兩個不同棋子移動到棋盤上的同一位置，也無法阻止其他違反規則的走法。視具體規則而定，有時可以使用唯一性約束；否則就很容易發生寫偏差。
...
2. 應用程式碼根據第一次查詢的結果決定下一步：繼續執行，或者向使用者報告錯誤並中止。
...
一個事務的寫入改變了另一個事務中搜尋查詢的結果，這種現象稱為*幻讀*[^4]。快照隔離可以避免只讀查詢中的幻讀，但在上述讀寫事務中，幻讀可能引發格外棘手的寫偏差。ORM 生成的 SQL 也很容易遇到寫偏差[^50] [^51]。
...
VoltDB/H-Store、Redis 和 Datomic 等系統採用了序列執行事務的方式[^58] [^59] [^60]。為單執行緒執行而設計的系統有時反而比支援併發的系統效能更好，因為它省去了加鎖的協調開銷。不過，其吞吐量受限於單個 CPU 核。要充分利用這條執行緒，事務的組織方式必須不同於傳統形式。
...
{{< fig num="8-9" id="fig_transactions_stored_proc" src="/fig/ddia_0809.png" caption="互動式事務與儲存過程的差異（採用圖 8-8中的示例事務）。" class="ddia-figure ddia-figure--wide" width="2658" height="1535" />}}
...
* 如果事務 A 已寫入某個物件，而事務 B 想讀取該物件，B 必須等 A 提交或中止後才能繼續。（2PL 不允許像{{< xref fig="8-4" page="/ch8" anchor="fig_transactions_read_committed" >}}圖 8-4{{< /xref >}}那樣讀取物件的舊版本。）
...
在會議室預訂的例子中，如果一個事務已經搜尋某個房間在特定時段內的現有預訂（參見{{< xref page="/ch8" anchor="fig_transactions_meeting_rooms" >}}示例 8-2{{< /xref >}}），另一個事務就不能併發插入或更新同一房間、同一時段的預訂。（併發預訂其他房間，或者預訂同一房間但互不影響的其他時段，則沒有問題。）
...
在{{< xref fig="8-10" page="/ch8" anchor="fig_transactions_detect_mvcc" >}}圖 8-10{{< /xref >}}中，事務 43 看到 Aaliyah 的 `on_call = true`，因為修改其值班狀態的事務 42 尚未提交。但等事務 43 準備提交時，事務 42 已經提交。這意味著，讀取一致快照時被忽略的寫入現在已經生效，事務 43 的前提不再成立。如果寫入者插入的是此前不存在的資料，情況還會更複雜（參見[“導致寫偏差的幻讀”](/tw/ch8#sec_transactions_phantom)）。[“檢測影響先前讀取的寫入”](/tw/ch8#sec_detecting_writes_affect_reads)將介紹 SSI 如何檢測幻寫。

{{< fig num="8-10" id="fig_transactions_detect_mvcc" src="/fig/ddia_0810.png" caption="檢測事務何時從 MVCC 快照讀取過時值。" class="ddia-figure ddia-figure--standard" width="2658" height="1613" />}}
...
第二種情況，是資料被讀取後又遭另一個事務修改，如{{< xref fig="8-11" page="/ch8" anchor="fig_transactions_detect_index_range" >}}圖 8-11{{< /xref >}}所示。

{{< fig num="8-11" id="fig_transactions_detect_index_range" src="/fig/ddia_0811.png" caption="在可序列化快照隔離中，檢測一個事務何時修改了另一個事務讀過的資料。" class="ddia-figure ddia-figure--standard" width="2658" height="1654" />}}
...
在{{< xref fig="8-11" page="/ch8" anchor="fig_transactions_detect_index_range" >}}圖 8-11{{< /xref >}}中，事務 42 和 43 都查詢了班次 `1234` 的值班醫生。如果 `shift_id` 上有索引，資料庫就能利用索引項 1234 記錄事務 42 和 43 讀過這項資料。（沒有索引時，也可以在表級別跟蹤。）這些資訊只需保留一段時間：等事務結束（提交或中止），並且所有與之併發的事務也結束後，資料庫就可以忘掉它讀過哪些資料。

事務寫入資料庫時，必須在索引中查詢近期讀過受影響資料的其他事務。這個過程類似為受影響的鍵範圍取得寫鎖，但它不會一直阻塞到讀事務提交，而是像警戒線一樣：只負責通知相關事務，它們讀過的資料可能已經過時。

在{{< xref fig="8-11" page="/ch8" anchor="fig_transactions_detect_index_range" >}}圖 8-11{{< /xref >}}中，事務 43 通知事務 42：它先前讀過的資料已經過時；事務 42 也反過來通知事務 43。事務 42 率先提交併獲得成功：雖然事務 43 的寫入影響了事務 42，但事務 43 尚未提交，所以寫入還未生效。等事務 43 準備提交時，事務 42 的衝突寫入已經提交，因此事務 43 必須中止。
...
對於只在單個資料庫節點上執行的事務，原子性通常由儲存引擎實現。客戶端請求資料庫節點提交時，資料庫先將事務寫入持久化（通常寫入預寫日誌；參見[“使 B 樹可靠”](/tw/ch4#sec_storage_btree_wal)），再向磁碟日誌追加一條提交記錄。如果資料庫在此過程中崩潰，節點重啟後便從日誌恢復事務：若提交記錄在崩潰前已經成功落盤，事務視為已提交；否則，該事務的所有寫入都會回滾。
...
此時，僅僅向所有節點傳送提交請求，讓每個節點獨立提交事務並不夠。如{{< xref fig="8-12" page="/ch8" anchor="fig_transactions_non_atomic" >}}圖 8-12{{< /xref >}}所示，提交很容易在一部分節點上成功，卻在另一部分節點上失敗：

* 某些節點可能檢測到約束違規或衝突，因而必須中止，其他節點卻能成功提交；
* 某些提交請求可能在網路中丟失，最終因超時而中止，其他請求卻順利抵達；
* 某些節點可能在提交記錄完全寫入前崩潰，恢復時回滾事務，其他節點卻成功提交。

{{< fig num="8-12" id="fig_transactions_non_atomic" src="/fig/ddia_0812.png" caption="當事務涉及多個資料庫節點時，它可能在某些節點上提交，在其他節點上失敗。" class="ddia-figure ddia-figure--wide" width="2658" height="998" />}}
...
2PC 的基本流程如{{< xref fig="8-13" page="/ch8" anchor="fig_transactions_two_phase_commit" >}}圖 8-13{{< /xref >}}所示。單節點事務只需一次提交請求，2PC 則把提交或中止過程分成兩個階段，名稱也由此而來。

{{< fig num="8-13" id="fig_transactions_two_phase_commit" src="/fig/ddia_0813.png" caption="兩階段提交（2PC）的成功執行。" class="ddia-figure ddia-figure--wide" width="2880" height="969" />}}
...
{{< fig num="8-14" id="fig_transactions_2pc_crash" src="/fig/ddia_0814.png" caption="參與者投票“是”之後，協調者崩潰。資料庫 1 不知道該提交還是中止。" class="ddia-figure ddia-figure--wide" width="2880" height="1006" />}}
...
事務提交或中止之前，資料庫不能釋放這些鎖（如{{< xref fig="8-13" page="/ch8" anchor="fig_transactions_two_phase_commit" >}}圖 8-13{{< /xref >}}中的陰影部分所示）。所以使用兩階段提交時，事務必須在整個存疑期間持鎖。協調者若崩潰後要花 20 分鐘重啟，鎖就會持有 20 分鐘；協調者日誌若因故徹底丟失，鎖甚至會永遠保留——至少要一直等到管理員手動解決問題。
...
本章尤其深入地探討了併發控制。我們介紹了幾種廣泛使用的隔離級別，特別是*讀已提交*、*快照隔離*（有時稱為*可重複讀*）和*可序列化*，並透過各種競態條件來刻畫它們。{{< xref tbl="8-1" page="/ch8" anchor="ch_transactions_isolation_levels" >}}表 8-1{{< /xref >}}彙總了這些內容：

{{< tbl num="8-1" id="ch_transactions_isolation_levels" caption="各種隔離級別下可能發生的異常彙總。" >}}
