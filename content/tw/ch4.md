...
儘管事實表通常有 100 多列，但典型的資料倉儲查詢一次只訪問其中 4、5 列（分析查詢很少需要 `"SELECT *"`）[^52]。以 {{< xref eg="4-1" page="/ch4" anchor="fig_storage_analytics_query" >}}示例 4-1{{< /xref >}} 為例：它會訪問大量行（2024 年中每一筆水果或糖果銷售記錄），卻只需要 `fact_sales` 表中的三列：`date_key`、`product_sk` 和 `quantity`。其他所有列都被查詢忽略了。

{{< eg num="4-1" id="fig_storage_analytics_query" caption="分析人們在一週中的哪一天更傾向於購買新鮮水果或糖果" >}}
```sql
SELECT
