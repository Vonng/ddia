...
如果你需要的只是擴展至更高的 *負載（load）*，最簡單的方法就是購買更強大的機器（有時稱為 *垂直擴展*，即 vertical scaling，或 *向上擴展*，即 scale up）。許多處理器，記憶體和磁碟可以在同一個作業系統下相互連線，快速的相互連線允許任意處理器訪問記憶體或磁碟的任意部分。在這種 *共享記憶體架構（shared-memory architecture）* 中，所有的元件都可以看作一臺單獨的機器。
...
另一種方法是 *共享磁碟架構（shared-disk architecture）*，它使用多臺具有獨立處理器和記憶體的機器，但將資料儲存在機器之間共享的磁碟陣列上，這些磁碟透過快速網路連線。這種架構用於某些資料倉儲，但競爭和鎖定的開銷限制了共享磁碟方法的可擴展性 [^2]。
...
相比之下，*無共享架構* [^3]（shared-nothing architecture，有時被稱為 *水平擴展*，即 horizontal scaling，或 *向外擴展*，即 scaling out）已經相當普及。
...
{{< fig num="II-1" id="fig_replication_partitioning" src="/v1/ddia_part-ii_01.png" caption="一個資料庫切分為兩個分割槽，每個分割槽都有兩個副本" class="ddia-figure ddia-figure--wide" width="2880" height="1606" />}}
