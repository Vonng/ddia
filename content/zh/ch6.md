...
在数据库中，让一次冲突阻塞整个复制过程，直到有人解决，显然并不现实。数据库通常会保存一条记录的所有并发写入值——例如 {{< xref fig="6-9" page="/ch6" anchor="fig_replication_write_conflict" >}}图 6-9{{< /xref >}} 中的 B 和 C。这些值有时称为 *兄弟值*（*sibling*）。下次查询该记录时，数据库返回 *全部* 值，而不只是最新的一个。随后可以任意选择解决办法：在应用代码中自动处理（例如把 B 与 C 拼成“B/C”），或询问用户；最后再向数据库写回一个新值，消解冲突。
