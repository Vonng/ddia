...
如果要让用户搜索车辆，并按颜色与品牌筛选，就需要在 `color` 和 `make` 上建立二级索引（在文档数据库中它们是字段，在关系数据库中则是列）。声明索引后，数据库会自动维护它。例如，每增加一辆红色汽车，所在分片就会自动把它的 ID 加入索引条目 `color:red` 对应的 ID 列表。正如 [第 4 章](/ch4#ch_storage) 所述，这种 ID 列表也称为 *倒排列表*（*postings list*）。
