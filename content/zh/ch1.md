...
但是，随着应用要实现的目标越来越高，挑战也随之而来。数据库系统种类繁多，特性各异，适用目的也各不相同——该选择哪一种？缓存有不同的做法，搜索索引也有多种构建方式，诸如此类——该如何权衡？你必须判断哪些工具和方法最适合手头的任务；而当一项工作无法由单个工具独力完成时，把多个工具组合起来也可能很困难。
...
这类查询产生的报表是商业智能的重要依据，可以帮助管理层决定下一步行动。为了把这种数据库使用模式与事务处理区分开来，人们称之为 *联机分析处理*（OLAP）[^5]。OLTP 与分析之间的界线并不总是泾渭分明，但[表 1-1](/ch1#tab_oltp_vs_olap)列出了二者的一些典型特征。

{{< figure id="tab_oltp_vs_olap" title="表 1-1. 事务型系统与分析型系统的特征比较" class="w-full my-4" >}}
...
数据仓库保存着企业各个 OLTP 系统中数据的只读副本。数据先从 OLTP 数据库中抽取出来（通过定期转储或连续的更新流），再转换成便于分析的模式并加以清理，最后载入数据仓库。把数据送入数据仓库的这一过程称为 *提取—转换—加载*（ETL），如[图 1-1](/ch1#fig_dwh_etl)所示。有时也会对调 *转换* 与 *加载* 两步的顺序，即先加载数据，再在数据仓库内转换；这便成了 *ELT*。

{{< figure src="/fig/ddia_0101.png" id="fig_dwh_etl" caption="图 1-1. 将数据通过 ETL 导入数据仓库的简化示意图。" class="w-full my-4" >}}
...
对软件来说，有两个重要决定：由谁来构建，又由谁来部署。两项工作都可以按不同程度外包，从而形成[图 1-2](/ch1#fig_cloud_spectrum)所示的一条连续谱。一个极端是完全定制的软件，由你自行编写并在内部运行；另一个极端是广泛使用的云服务或软件即服务（SaaS）产品，由外部供应商开发和运维，你只能通过 Web 界面或 API 使用。

{{< figure src="/fig/ddia_0102.png" id="fig_cloud_spectrum" caption="图 1-2. 软件类型及其运维方式的连续谱。" class="w-full my-4" >}}
...
原则上，几乎任何可以自托管的软件都能以云服务的形式提供；事实上，许多流行的数据系统如今都有相应的托管服务。然而，从一开始就按云原生思路设计的系统已经展现出若干优势：在相同硬件上性能更好，故障恢复更快，能够迅速调整计算资源以匹配负载，并且可以支持更大的数据集 [^25] [^26] [^27]。[表 1-2](/ch1#tab_cloud_native_dbs)列出了两类系统的一些例子。

{{< figure id="tab_cloud_native_dbs" title="表 1-2. 自托管数据库系统与云原生数据库系统示例" class="w-full my-4" >}}
