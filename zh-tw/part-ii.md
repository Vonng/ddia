...
* 複製（Replication）
  ...
  在幾個不同的節點上儲存資料的相同副本，可能放在不同的位置。複製提供了冗餘：如果一些節點不可用，剩餘的節點仍然可以提供資料服務。複製也有助於改善效能。[第六章](ch6.md) 將討論複製。

* 分割槽 (Partitioning)

  將一個大型資料庫拆分成較小的子集（稱為 **分割槽**，即 partitions），從而不同的分割槽可以指派給不同的 **節點**（nodes，亦稱 **分片**，即 sharding）。[第七章](ch7.md) 將討論分割槽。
...
理解了這些概念，就可以開始討論在分散式系統中需要做出的困難抉擇。[第八章](ch8.md) 將討論 **事務（Transaction）**，這對於瞭解資料系統中可能出現的各種問題，以及我們可以做些什麼很有幫助。[第九章](ch9.md) 和 [第十章](ch10.md) 將討論分散式系統的根本侷限性。
...
| 上一章                 | 目錄                     | 下一章              |
|---------------------|------------------------|------------------|
| [第五章：編碼與演化](ch5.md) | [設計資料密集型應用](README.md) | [第六章：複製](ch6.md) |