...
我们将从 [第十一章](ch11.md) 开始，研究例如 MapReduce 这样 **面向批处理（batch-oriented）** 的数据流系统。对于建设大规模数据系统，我们将看到，它们提供了优秀的工具和思想。[第十一章](ch11.md) 将把这些思想应用到 **流式数据（data streams）** 中，使我们能用更低的延迟完成同样的任务。[第十二章](ch12.md) 将对本书进行总结，探讨如何使用这些工具来构建可靠，可伸缩和可维护的应用。
...
| 上一章                   | 目录                     | 下一章                 |
|-----------------------|------------------------|---------------------|
| [第十章：一致性与共识](ch10.md) | [设计数据密集型应用](README.md) | [第十一章：批处理](ch11.md) |